...
export default function About() {
  ...
  return (
    <section id="about" className="py-24 px-[5%] bg-[#000b1d] relative overflow-hidden">
      {/* Background Decor */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
          ...
          <div className="hidden lg:flex justify-center relative h-[360px]">
            <div className="relative w-[360px] h-[360px] flex items-center justify-center">
              ...
              />
              <div className="absolute inset-[40px] rounded-full border-[1.5px] border-white/5" />
              <div className="absolute inset-[65px] rounded-full bg-white/5 border border-white/10 backdrop-blur-3xl shadow-2xl" />
              
              {/* Orbiting Dots */}
              {[
                { color: "bg-blue", delay: 0, shadow: "shadow-[0_0_15px_rgba(11,128,190,0.5)]" },
                { color: "bg-white", delay: -2.3, shadow: "shadow-[0_0_15px_rgba(255,255,255,0.3)]" },
                { color: "bg-blue-400", delay: -4.6, shadow: "shadow-[0_0_15px_rgba(96,165,250,0.5)]" },
              ].map((dot, i) => (
                <motion.div
                  key={i}
                  animate={{ rotate: 360 }}
              ...
              <div className="relative z-10 w-40 h-40 bg-white/10 backdrop-blur-md rounded-full border border-white/20 shadow-2xl flex items-center justify-center p-6">
                <Image src="/logo.svg" alt="AAI Logo" width={120} height={120} className="brightness-110" />
              </div>
            </div>
          </div>
          ...
          >
            ...
            <h2 className="font-plus-jakarta text-[clamp(1.8rem,3.5vw,2.7rem)] font-extrabold leading-tight tracking-tight text-white mb-6">
              Supporting ideas that bring real change
            </h2>
            <div className="space-y-5">
              <p className="text-white/70 leading-relaxed font-medium text-[1.05rem]">
            ...
            <div className="mt-10">
              <div className="font-plus-jakarta font-bold text-white text-lg mb-5">We back:</div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {backItems.map((item, i) => (
                  <motion.div 
                    key={i}
                    whileHover={{ scale: 1.02, borderColor: "rgba(11,128,190,0.5)", backgroundColor: "rgba(255,255,255,0.08)" }}
                    className="flex items-start gap-3.5 p-4 bg-white/5 border border-white/10 rounded-2xl transition-all"
                  >
                    <div className="w-9 h-9 rounded-xl bg-blue/20 border border-blue/30 flex items-center justify-center shrink-0 text-blue-400">
                      <item.icon size={18} />
                    </div>
                    <p className="text-white/80 text-[0.85rem] leading-snug font-medium">{item.text}</p>
                  </motion.div>
                ))}
              </div>
            </div>
